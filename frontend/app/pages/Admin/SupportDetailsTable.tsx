...
const SupportAdminTable: React.FC = () => {
  ...
  const fetchSupportAdmins = async () => {
    try {
      ...
      if (err.response?.status === 401) {
        ...
        setError(
          err.response?.data?.message || "Failed to fetch support admins"
        );
      }
    } finally {
      setLoading(false);
  ...
  const handleDelete = async (id: string) => {
    try {
      const confirmDelete = window.confirm(
        "Are you sure you want to delete this support admin?"
      );
      ...
      if (err.response?.status === 403) {
        alert(
          "Permission denied: You are not authorized to delete support admins"
        );
  ...
  const handleSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const filtered = supportAdmins.filter(
      (admin) =>
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow-md">
      ...
      <div className="flex justify-between items-center mt-4 text-sm text-gray-500">
        <div>
          Showing {filteredAdmins.length} of {supportAdmins.length} support
          admins
        </div>
      </div>
    </div>
